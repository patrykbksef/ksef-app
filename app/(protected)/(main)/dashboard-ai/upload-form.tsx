...
import { FileCheck2, FileUp, LoaderCircle, Sparkles, Trash2 } from "lucide-react";
    ...
    <form onSubmit={onSubmit} className="space-y-5">
      ...
      <div
        ...
        className={cn(
          ...
          dragOver && !disabled && "scale-[1.01] border-violet-500 bg-violet-500/5 shadow-sm",
          file && "border-emerald-500/50 bg-emerald-500/5",
          disabled && "pointer-events-none opacity-50",
        )}
      >
        <div className={cn(
          "mx-auto flex size-14 items-center justify-center rounded-2xl bg-violet-500/10 text-violet-700 dark:text-violet-300",
          file && "bg-emerald-500/10 text-emerald-600 dark:text-emerald-300",
        )}>
          {file ? <FileCheck2 className="size-7" /> : <Sparkles className="size-7" />}
        </div>
        <p className="mt-4 text-lg font-semibold">
          {file ? "Plik jest gotowy" : "Dodaj fakturę do analizy AI"}
        </p>
      ...
      <Button type="submit" size="lg" className="w-full" disabled={disabled || !file || pending}>
        ...
        {pending ? "Analiza AI…" : "Prześlij i odczytaj dane (AI)"}
      </Button>
    </form>
  );
}
